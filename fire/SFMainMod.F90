...
contains
  ...
  subroutine  crown_damage ( currentSite )
    ...
    use SFParamsMod, only: SF_val_min_moisture,  &
                           SF_val_mid_moisture,  &
                ...
                if (currentPatch%SH < (currentCohort%hite-currentCohort%hite*EDPftvarcon_inst%crown(currentCohort%pft))) then 
                   ...
                   if ((currentCohort%hite > 0.0_r8).and.(currentPatch%SH >=  &
                      ...
                      if (EDPftvarcon_inst%crown_fire(currentCohort%pft) > 0._r8) then
                         ...
                         ! I increase fraction_crown_burned based on the
                         ! moisture conditions. For now I use the
                         ! moisture variable already calculated for live
                         ! grasses: litter_moisture(lg_sf)
                         ! I then use an algorithm that in
                         ! subroutine ground_fuel_consumption
                         ! calculates burnt_frac_litter.
                         ...
                         ! 2. Low to medium moistures
                         else if (moist > SF_val_min_moisture(lg_sf) .and. moist <= SF_val_mid_moisture(lg_sf)) then
                            currentCohort%fraction_crown_burned =  &
                               max(currentCohort%fraction_crown_burned,  &
                                   min(1.0_r8,  &
                                       SF_val_low_moisture_Coeff(lg_sf) -  &
                                       SF_val_low_moisture_Slope(lg_sf) * moist))
                         ! For medium to high moistures.
                         else if (moist > SF_val_mid_moisture(lg_sf) .and. moist <= 1.0_r8) then
                            currentCohort%fraction_crown_burned =  &
                               max(currentCohort%fraction_crown_burned,  &
                                   min(1.0_r8,  &
                                       SF_val_mid_moisture_Coeff(lg_sf) -  &
                                       SF_val_mid_moisture_Slope(lg_sf) * moist))
                         else
                            ! do not change fraction_crown_burned
                         end if
